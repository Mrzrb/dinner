  ...
  return (
    <div className="p-4 pb-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-xl font-semibold">新增菜谱</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addIngredient} className="text-sm text-neutral-900">
              + 添加
            </button>
          </div>
            ...
            {ingredients.map((ing, i) => (
              <div key={i} className="flex gap-2">
                ...
                <input
                  ...
                  placeholder="用量"
                  className="w-20 px-3 py-2 rounded-lg border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900"
                />
                <input
                  type="text"
                  value={ing.unit}
                  ...
                  placeholder="单位"
                  className="w-16 px-3 py-2 rounded-lg border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900"
                />
                <button
                  type="button"
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button type="button" onClick={addStep} className="text-sm text-neutral-900">
              + 添加
            </button>
          </div>
          <div className="space-y-2">
            {steps.map((step, i) => (
              <div key={i} className="flex gap-2 items-start">
                <span className="w-6 h-6 rounded-full bg-neutral-200 text-neutral-700 text-xs flex items-center justify-center flex-shrink-0 mt-2">
                  {i + 1}
                </span>
                <textarea
                  value={step.content}
                  ...
                  placeholder={`步骤 ${i + 1}`}
                  rows={2}
                  className="flex-1 px-3 py-2 rounded-lg border border-neutral-200 text-sm focus:outline-none focus:ring-2 focus:ring-neutral-900 resize-none"
                />
                <button
                  type="button"
                  onClick={() => removeStep(i)}
                  className="text-neutral-300 hover:text-red-500 mt-2"
                >
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                  </svg>
